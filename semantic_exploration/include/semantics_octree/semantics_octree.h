...
class SemanticsOcTree : public OccupancyOcTreeBase<SemanticsOcTreeNode<SEMANTICS> >
{
  public:
    ...
    SemanticsOcTree* create() const
    {
        return new SemanticsOcTree(this->resolution);
    }
    ...
    bool isUseSemanticColor()
    {
        return this->root->use_semantic_color;
    }

    void setUseSemanticColor(bool use);

    // set node color at given key or coordinate. Replaces previous color.
    SemanticsOcTreeNode<SEMANTICS>* setNodeColor(const OcTreeKey& key, uint8_t r, uint8_t g,
                                                 uint8_t b);

    SemanticsOcTreeNode<SEMANTICS>* setNodeColor(float x, float y, float z, uint8_t r, uint8_t g,
                                                 uint8_t b)
    {
        OcTreeKey key;
    ...
    SemanticsOcTreeNode<SEMANTICS>* averageNodeColor(const OcTreeKey& key, uint8_t r, uint8_t g,
                                                     uint8_t b);

    /// Integrate color measurement (RGB) at given coordinate. Average with previous color
    SemanticsOcTreeNode<SEMANTICS>* averageNodeColor(float x, float y, float z, uint8_t r,
                                                     uint8_t g, uint8_t b)
    {
        OcTreeKey key;
        if (!this->coordToKeyChecked(point3d(x, y, z), key))
