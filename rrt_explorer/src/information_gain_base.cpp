...
#ifndef INFORMATIONGAINBASE_HPP_
#define INFORMATIONGAINBASE_HPP_
#include <thread>
#include <chrono>
#include <cstdlib>
...
rrtNBV::InformationGainBase::InformationGainBase(mesh::StlMesh * mesh, volumetric_mapping::OctomapManager * manager)
{
  mesh_ = mesh;
  manager_ = manager;
  kdTree_ = kd_create(3);
...
void rrtNBV::InformationGainBase::setStateFromPoseMsg(const geometry_msgs::PoseWithCovarianceStamped& pose)
{
...
void rrtNBV::InformationGainBase::setStateFromPoseStampedMsg(const geometry_msgs::PoseStamped& pose)
{
  // Get latest transform to the planning frame and transform the pose
  ...
  tf::StampedTransform transform;
      ...
      std::cout << "received Pose" << pose.pose.position.x << " " << pose.pose.position.y << "  " << pose.pose.position.z << "  " << pose.pose.orientation.x << "  " << pose.pose.orientation.y << "  " << pose.pose.orientation.z << pose.pose.orientation.w<< std::endl ; 
  ...
  std::cout << "Frame NAV " << pose.header.frame_id << std::endl ; 
  std::cout << "params_.navigationFrame_ NAV " << pose.header.frame_id << std::endl ; 
  ...
  root_[2] = position.z();
  root_[3] = tf::getYaw(quat);
  std::cout << root_[0] << " " << root_[1] << "  " << root_[2] << "  " << root_[3] << std::endl ; 
  ...
  if (ros::Time::now().toSec() - logThrottleTime > params_.log_throttle_) {
    ...
    std::cout << poseMsg.pose.position.x << " " << poseMsg.pose.position.y << "  " << poseMsg.pose.position.z << "  " << poseMsg.pose.orientation.x << "  " << poseMsg.pose.orientation.y << "  " << poseMsg.pose.orientation.z << poseMsg.pose.orientation.w<< std::endl ; 

  // Update the inspected parts of the mesh using the current positio
  if (ros::Time::now().toSec() - inspectionThrottleTime_[0] > params_.inspection_throttle_) {
    inspectionThrottleTime_[0] += params_.inspection_throttle_;
    if (mesh_) {
      geometry_msgs::Pose poseTransformed;
...
void rrtNBV::InformationGainBase::setStateFromOdometryMsg(const nav_msgs::Odometry& pose)
{
  // Get latest transform to the planning frame and transform the pose
  static tf::TransformListener listener;
  tf::StampedTransform transform;
...
bool rrtNBV::InformationGainBase::iterate(int iterations)
{
        ROS_INFO("&&&&&&&&&&&&&&&&&&&iterate&&&&&&&&&&&&&&&&&&&&&&&&");
...
void rrtNBV::InformationGainBase::initialize()
{
    ROS_INFO("&&&&&&&&&&&&&&&&&&&Initialize&&&&&&&&&&&&&&&&&&&&&&&&");
  ...
  iterationCount_++;
  ROS_INFO("rootNode_ %f , %f , %f , %f", rootNode_->state_.x() ,rootNode_->state_.y(),rootNode_->state_.z() ,rootNode_->state_.z());
...
std::vector<geometry_msgs::Pose> rrtNBV::InformationGainBase::getBestEdge(std::string targetFrame)
{
...
double rrtNBV::InformationGainBase::getBestGain()
{
  return bestGain_;
}

double rrtNBV::InformationGainBase::gain(StateVec state)
{
...
std::vector<geometry_msgs::Pose> rrtNBV::InformationGainBase::getPathBackToPrevious(
...
geometry_msgs::Pose rrtNBV::InformationGainBase::getBestEdgePoint(std::string targetFrame)
{
  ...
  //ret.orientation.x = bestNode_->state_[3] ;
  tf::Quaternion q = tf::createQuaternionFromYaw(bestNode_->state_[3]) ;
  
  ret.orientation.x = q[0] ;
  ret.orientation.y = q[1] ;
  ret.orientation.z = q[2] ;
  ret.orientation.w = q[3] ;
  std::cout << "bestNode_->state_[3] " << bestNode_->state_[3] << " " << q[0] << " " << q[1] << " " << q[2] << "  " << q[3] <<  std::endl ; 

  return ret;
}

void rrtNBV::InformationGainBase::memorizeBestBranch()
{
...
void rrtNBV::InformationGainBase::clear()
{
  delete rootNode_;
  rootNode_ = NULL;
...
void rrtNBV::InformationGainBase::publishNode(Node * node)
{
  visualization_msgs::Marker p;
  p.header.stamp = ros::Time::now();
  p.header.seq = g_ID_;
...
std::vector<geometry_msgs::Pose> rrtNBV::InformationGainBase::samplePath(StateVec start, StateVec end,
                                                             std::string targetFrame)
{
  std::vector<geometry_msgs::Pose> ret;
  static tf::TransformListener listener;
