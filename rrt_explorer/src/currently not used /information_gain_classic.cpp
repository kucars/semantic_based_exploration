...
#ifndef INFORMATIONGAINCLASSIC_HPP_
#define INFORMATIONGAINCLASSIC_HPP_
#include <thread>
#include <chrono>
#include <cstdlib>
...
rrtNBV::InformationGainClassic::InformationGainClassic()
  : rrtNBV::InformationGainBase::InformationGainBase()
{}

rrtNBV::InformationGainClassic::InformationGainClassic(mesh::StlMesh * mesh, volumetric_mapping::OctomapManager * manager)
{ 
    ROS_INFO("****************************** Pure Entropy 3 ***************************************");
...
void rrtNBV::InformationGainClassic::initialize()
{
  ...
  ROS_INFO("PoseMsg In iterate %f %f  %f %f %f %f %f ", poseMsg.pose.position.x,poseMsg.pose.position.y,poseMsg.pose.position.z,poseMsg.pose.orientation.x,poseMsg.pose.orientation.y,poseMsg.pose.orientation.z,poseMsg.pose.orientation.w);

}

bool rrtNBV::InformationGainClassic::iterate(int iterations)
{
            ROS_INFO("&&&&&&&&&&&&&&&&&&&iterate&&&&&&&&&&&&&&&&&&&&&&&&");
...
double rrtNBV::InformationGainClassic::gain(StateVec state)
{
      ROS_INFO("****************************** GAIN *******************************************") ; 
    ...
    for (vec[1] = std::max(state[1] - params_.gainRange_, params_.minY_);
      ...
      for (vec[2] = std::max(state[2] - params_.gainRange_, params_.minZ_);
        ...
        entropy= - p * std::log(p) - ((1-p) * std::log(1-p));
         ...
         std::cout << "gain" << gain <<  std::endl << std::flush ;
